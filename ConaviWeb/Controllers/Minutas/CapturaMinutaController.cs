...
namespace ConaviWeb.Controllers.Minutas
{
    ...
    public class CapturaMinutaController : Controller
    {
        ...
        public CapturaMinutaController(IMinutaRepository minutaRepository, IWebHostEnvironment environment, IUserRepository userRepository)
        {
        ...
        {
            var reuniones = await _minutaRepository.GetReunionMinuta();
        ...
        public async Task<JsonResult> GetReunionAsync(int idReunion, string tipo)
        {
            
            if (tipo != "RT") { 
                ReunionResponse reunion = await _minutaRepository.GetRMinuta(idReunion);
                return Json(reunion);
            }
            else
            {
                ReunionTitular reunion = await _minutaRepository.GetRTMinuta(idReunion);
                return Json(reunion);
            }
        ...
        public async Task<IActionResult> CreateMinutaAsync(Minuta minuta)
        {
            ...
            if(minuta.IdReunion != 0)
                await MinutaRPdfAsync(minuta.IdReunion);
            else
                await MinutaRTPdfAsync(minuta.IdRTitular);

            TempData["Alert"] = AlertService.ShowAlert(Alerts.Success, "Se agrego la minuta con exito");
            return RedirectToAction("Index");

        }
        public async Task MinutaRPdfAsync(int id)
        {
            ...
            var pathCarta = Path.Combine(_environment.WebRootPath, "doc", "RMinutas");
            ...
            foreach (var item in acuerdos)
            {
                ...
                tableAcu.AddCell(new Paragraph(item.FechaTermino == null ? "N/A": item.FechaTermino)).SetFont(fonte).SetFontSize(8);
            }
        ...
        public async Task MinutaRTPdfAsync(int id)
        {
            //var acuerdos = await _minutaRepository.GetAcuerdos(id);
            var minuta = await _minutaRepository.GetMinutaRT(id);
            var fecha = String.Concat(minuta.FchCreate.Day, " de ", minuta.FchCreate.ToString("MMMM", CultureInfo.CreateSpecificCulture("es")), " de ", minuta.FchCreate.Year);
            var _header = System.IO.Path.Combine(_environment.WebRootPath, "img", "SEDATU_Logo_(2024).png");
            var _footer = System.IO.Path.Combine(_environment.WebRootPath, "img", "footerConavi.png");
            var pathCarta = Path.Combine(_environment.WebRootPath, "doc", "RTMinutas");
            if (!Directory.Exists(pathCarta))
            {
                Directory.CreateDirectory(pathCarta);
            }
        ...
        public IActionResult Download(int id)
        {
            ...
            var path = Path.Combine(_environment.WebRootPath, "doc", "RMinutas", "minuta_" + id + ".pdf");
            ...
            return File(fs, "application/octet-stream", "minuta_r" + id + ".pdf");
        }
        [Route("DownloadRT/{id?}")]
        public IActionResult DownloadRT(int id)
        {
            // Since this is just and example, I am using a local file located inside wwwroot
            // Afterwards file is converted into a stream
            var path = Path.Combine(_environment.WebRootPath, "doc", "RTMinutas", "minuta_" + id + ".pdf");
            if (!System.IO.File.Exists(path))
            {
                return RedirectToAction("Index", "ListaRTitular");
            }
            var fs = new FileStream(path, FileMode.Open);
            // Return the file. A byte array can also be used instead of a stream
            return File(fs, "application/octet-stream", "minuta_rt" + id + ".pdf");
        }
    }
}
